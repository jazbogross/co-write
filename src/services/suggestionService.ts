...
export const approveSuggestion = async (
  ...
  try {
    ...
    // Transform all other pending suggestions to reflect the approved change
    try {
      const { data: others } = await supabase
        .from('script_suggestions')
        .select('id, delta_diff')
        .eq('script_id', scriptId)
        .eq('status', 'pending')
        .neq('id', suggestionId);
      ...
      if (others && others.length > 0) {
        for (const other of others) {
          let otherOps;
          if (other.delta_diff && typeof other.delta_diff === 'object' && Array.isArray(other.delta_diff.ops)) {
            otherOps = other.delta_diff.ops;
          } else {
            continue;
          }
          const otherDelta = new Delta(otherOps);
          // Transform other deltas to apply after the approved diff
          const transformed = diffDelta.transform(otherDelta, true);
          const transformedJson = toJSON(transformed);
          await supabase
            .from('script_suggestions')
            .update({ delta_diff: transformedJson, updated_at: new Date().toISOString() })
            .eq('id', other.id);
        }
      }
    } catch (transformErr) {
      console.warn('Failed to transform remaining suggestions:', transformErr);
    }
...
/**
 * Fetch approved diffs since a given timestamp, ordered chronologically
 */
export const fetchApprovedDiffsSince = async (
  scriptId: string,
  since: string
): Promise<{ delta: Delta; updatedAt: string }[]> => {
  try {
    const { data, error } = await supabase
      .from('script_suggestions')
      .select('delta_diff, updated_at, status')
      .eq('script_id', scriptId)
      .eq('status', 'approved')
      .gte('updated_at', since)
      .order('updated_at', { ascending: true });
    if (error) throw error;
    const list: { delta: Delta; updatedAt: string }[] = [];
    (data || []).forEach((row: any) => {
      if (row?.delta_diff?.ops && row.updated_at) {
        list.push({ delta: new Delta(row.delta_diff.ops), updatedAt: row.updated_at });
      }
    });
    return list;
  } catch (e) {
    console.error('Error fetching approved diffs since', since, e);
    return [];
  }
};
