...
import { InlineSuggestionControls } from '@/components/suggestions/InlineSuggestionControls';
...
export const DeltaTextEditor: React.FC<DeltaTextEditorProps> = ({
  ...
  const {
    ...
    editorContent,
    quillRef,
    showDiff: isAdmin && hasPendingSuggestions
  });
  const diffActive = isAdmin && (hasPendingSuggestions || overlayActive);
  ...
  return (
    <div className={`flex flex-col border rounded-lg overflow-hidden relative ${diffActive ? 'quill-diff-editor' : ''}`}>
      <TextEditorActions
        isAdmin={isAdmin}
      ...
      {diffActive && (
        <InlineSuggestionControls
          position={hoverControlsPos}
          suggestionId={hoveredSuggestionId}
